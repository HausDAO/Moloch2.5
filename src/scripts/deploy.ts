...
import { NftEscrowMinion } from '../types/NftEscrowMinion'
...
async function main() {
  ...
  const EscrowMinionTemplate = await ethers.getContractFactory('NFTEscrowMinion')
  ...
  const escrowMinion = (await EscrowMinionTemplate.deploy()) as NftEscrowMinion
